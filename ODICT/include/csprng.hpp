...
namespace duthomhas
{
  ...
  public:
    ...
    csprng(Iterator begin, Iterator end) : internal(csprng_create()),
                                           sseq(internal, std::distance(begin, end))
    {
    }

    template <typename T>
    csprng(std::initializer_list<T> xs) : internal(csprng_create()),
    ...
    struct sseq_type
    {
      ...
      sseq_type(CSPRNG &internal, std::size_t seed_seq_size) : internal(internal),
  ...
  public:
    ...
    csprng(const csprng &that) : internal(csprng_create()),
